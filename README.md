...
🌐 **Language / Til / Язык:** &nbsp; [🇬🇧 English](#-project-overview) &nbsp;|&nbsp; [🇺🇿 O'zbekcha](#-loyiha-haqida) &nbsp;|&nbsp; [🇷🇺 Русский](#-о-проекте)
...
# 🇷🇺 Прогнозирование Объёма Платежей в Узбекистане

> **Система прогнозирования временных рядов производственного уровня, предсказывающая ежедневные объёмы транзакций в быстрорастущей платёжной экосистеме Узбекистана — построена на XGBoost, CatBoost, линейной регрессии, FastAPI и Docker.**

---

## 📌 О Проекте

Рынок цифровых платежей Узбекистана — один из самых быстрорастущих в Центральной Азии. Система быстрых платежей обработала более **$10,2 млрд транзакций к середине 2025 года**, а платформы Payme, Click, Uzum и Paynet в совокупности обслуживают десятки миллионов пользователей. Точное прогнозирование объёмов транзакций критически важно для:

- **Планирования мощностей** — сколько серверов потребуется платёжной платформе завтра?
- **Управления ликвидностью** — какой объём оборотных средств должен поддерживать банк?
- **Обнаружения аномалий** — подозрительно ли низок или высок сегодняшний объём?
- **Бизнес-аналитики** — какие сезоны, праздники и экономические события определяют платёжное поведение?

Проект создаёт полный конвейер прогнозирования производственного уровня — от сырых данных до развёрнутого REST API.
...
| Навык | Реализация |
|---|---|
| Разработка признаков временных рядов | Признаки с задержкой, скользящие окна, циклическое синус/косинус кодирование |
| Доменные знания по Узбекистану | Рамадан, Навруз, циклы дней зарплаты, влияние COVID |
| Сравнение моделей | Линейная регрессия vs XGBoost vs CatBoost |
| Интеграция реальных данных | Актуальные курсы USD/UZS от API Центрального банка Узбекистана |
| Интерпретируемость модели | Анализ важности признаков методом SHAP |
| ML-сервис производственного уровня | REST-эндпоинт FastAPI с валидацией входных данных |
| Контейнеризация | Развёртывание через Docker + docker-compose |
| Строгость оценки | MAE, RMSE, SMAPE по временным сегментам |
...
```
API Центрального банка Узбекистана (cbu.uz)
        │
        ▼
┌─────────────────────────────────────────────────────┐
│              СЛОЙ ДАННЫХ                             │
│  Синтетические объёмы платежей + реальные курсы CBU │
│  (2019–2024, 2 192 ежедневные записи)               │
└─────────────────┬───────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────┐
│         КОНВЕЙЕР РАЗРАБОТКИ ПРИЗНАКОВ                │
│  • Временны́е: кодирование дня/недели/месяца/кварт. │
│  • Циклические: синус/косинус для сезонности         │
│  • Календарь УЗ: Рамадан, Навруз, дни зарплаты      │
│  • Признаки с задержкой: t-1, t-7, t-14, t-30       │
│  • Скользящая статистика: среднее и СКО 7/14/30 дн. │
│  • Макро: курс USD/UZS, изменение курса за 7 дней   │
└─────────────────┬───────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────┐
...
│                                                      │
│  ┌─────────────┐  ┌──────────┐  ┌──────────────┐   │
│  │  Линейная   │  │ XGBoost  │  │  CatBoost    │   │
│  │ регрессия   │  │          │  │              │   │
│  │  (базовая)  │  │(ансамбль)│  │ (СНГ-решение)│   │
│  └─────────────┘  └──────────┘  └──────────────┘   │
│                                                      │
│      Обучение: 2019–2022 | Тест: 2023–2024          │
└─────────────────┬───────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────┐
│           ОЦЕНКА И ИНТЕРПРЕТАЦИЯ                     │
│  • MAE, RMSE, SMAPE для каждой модели               │
│  • Графики прогноза vs реальные значения             │
...
│                                                      │
│   POST /predict → FastAPI → Лучшая модель           │
│   GET  /health  → Проверка статуса                  │
│   GET  /models  → Доступные модели + метрики        │
│                                                      │
...
### Синтетические Данные о Платежах
Данные об объёмах транзакций **синтетически сгенерированы, но откалиброваны** по агрегированным показателям из статистических бюллетеней ЦБ и публичных отчётов о цифровой платёжной экосистеме Узбекистана. Ключевые закодированные паттерны:

- **Экспоненциальный рост** — отражает внедрение цифровых платежей в УЗ (500К → 3,6М транзакций в день, 2019–2024)
- **Недельная сезонность** — в выходные дни объём в Узбекистане ниже примерно на 22%
- **Эффект Рамадана** — снижение ~12% в священный месяц (даты смещаются каждый год)
- **Всплеск Навруза** — 20–23 марта, рост объёма ~35% (национальный праздник)
- **Циклы дней зарплаты** — 10-е и 25-е числа каждого месяца показывают всплеск ~22%
- **Провал COVID-19** — II квартал 2020 года показывает снижение объёма ~35%
- **Региональное распределение** — Ташкент (45%), Самарканд (20%), Фергана (15%), Наманган (10%), Андижан (10%)

### Реальные Данные Обменного Курса
Курсы USD/UZS получаются напрямую из **публичного API Центрального банка Узбекистана** (`cbu.uz/en/arkhiv-kursov-valyut/json/{date}/`) за период 2019–2024. Это даёт реальный макроэкономический сигнал — обесценивание валюты коррелирует со сдвигами в потребительском поведении.

**Диапазон курса:** ~8 400 → ~12 700 сум / 1 USD (2019–2024)

---

## 🔧 Разработка Признаков

28 признаков, разработанных из сырых дат и экономических сигналов:

**Временны́е признаки** — `days_elapsed`, `day_of_week`, `month`, `quarter`, `week_of_year`, `day_of_month`

**Циклическое кодирование** (та же техника, что используется в промышленных системах прогнозирования) — `month_sin/cos`, `day_of_week_sin/cos`, `day_of_month_sin/cos`. Это не позволяет модели считать декабрь (12) и январь (1) далёкими друг от друга.

**Флаги узбекского календаря** — `is_ramadan`, `is_navruz`, `is_payday`, `is_weekend`, `is_covid_dip`

**Признаки с задержкой** — `volume_lag_1`, `volume_lag_7`, `volume_lag_14`, `volume_lag_30` (что происходило 1/7/14/30 дней назад)

**Скользящая статистика** — скользящее среднее и стандартное отклонение за 7/14/30 дней (сигналы тренда + волатильности)

**Макропризнаки** — `usd_uzs_rate`, `usd_uzs_rate_lag7`, `usd_uzs_rate_change` (реальные данные ЦБ)
...
Стандартная МНК-регрессия. Устанавливает нижнюю границу производительности. Хорошо улавливает общий тренд, но с трудом справляется с нелинейными сезонными паттернами.

### 2. XGBoost
Градиентный бустинг на деревьях. Улавливает нелинейные взаимодействия между признаками (например, комбинация Рамадана + выходной день ведёт себя иначе, чем каждый из них по отдельности). Как правило, на 25–40% лучший SMAPE по сравнению с линейной базой на данных такого типа.

### 3. CatBoost
Библиотека градиентного бустинга от Яндекса — особенно актуальна для компаний СНГ-региона (технологии Яндекса глубоко встроены в технологическую экосистему Узбекистана). Нативно работает с категориальными признаками и зачастую превосходит XGBoost на табличных данных с меньшей настройкой гиперпараметров.

### Разделение на Обучение/Тест
- **Обучение:** 2019-01-01 → 2022-12-31 (4 года)
- **Тест:** 2023-01-01 → 2024-12-31 (2 года, никогда не видены при обучении)

Это **walk-forward** разделение — модель всегда тестируется на будущих данных, без случайного перемешивания (что вызвало бы утечку данных в задачах временных рядов).
...
|---|---|
| **MAE** | Средняя абсолютная ошибка в количестве транзакций |
| **RMSE** | Штрафует крупные ошибки сильнее |
| **SMAPE** | Симметричная процентная ошибка — независима от масштаба, сопоставима между моделями |

Результаты вычисляются как в целом, так и с разбивкой по годам, кварталам и специфическим для Узбекистана периодам (Рамадан, Навруз, COVID).
...
### `GET /models`
Возвращает доступные модели и их метрики на тестовой выборке (MAE, RMSE, SMAPE).
...
// Ответ
{
  ...
  "is_navruz": true,
  "is_ramadan": false,
  "confidence_note": "Обнаружен праздник Навруз — ожидается повышенный объём"
}
```
...
| ML-модели | scikit-learn, XGBoost, CatBoost |
| Интерпретируемость | SHAP |
| Разработка признаков | pandas, numpy |
| API | FastAPI + Uvicorn |
| Контейнеризация | Docker, docker-compose |
| Визуализация | matplotlib, seaborn |
| Источник данных | Публичный API ЦБ Узбекистана |

---

## 💡 Ключевые Выводы и Архитектурные Решения

**Почему не LSTM/Prophet?** XGBoost с тщательно разработанными признаками задержки стабильно превосходит модели глубокого обучения на табличных данных временных рядов такого масштаба. Prophet с трудом справляется с несколькими сезонными паттернами одновременно (недельный + Рамадан + Навруз).

**Почему именно CatBoost?** CatBoost разработан Яндексом, который имеет глубокие корни в инфраструктуре Узбекистана и рынков СНГ. Компании вроде Uzum, выросшие из этой экосистемы, знакомы с CatBoost и нередко предпочитают его в производственных развёртываниях.

**Почему синтетические данные?** Платёжные платформы Узбекистана (Payme, Click, Uzum) не публикуют детальные ежедневные данные о транзакциях. Синтетические данные откалиброваны по ежегодным агрегированным показателям, опубликованным ЦБ, и обогащены реальными макроэкономическими сигналами (реальные курсы USD/UZS). Это отражает реальную практику ML — специалисты по данным часто строят и валидируют конвейеры на синтетических данных до получения доступа к производственным.

---

## 📋 Актуальность для Рынка Труда Узбекистана

Проект непосредственно применим к инженерным позициям в:

- **Uzum** — маркетплейс + BNPL + цифровой банк с 20 млн ежемесячных пользователей, нуждающийся в прогнозировании спроса
- **Alif** — основной бизнес зависит от прогнозирования объёма транзакций для управления ликвидностью
- **Payme / Click / Paynet** — платёжные платформы, нуждающиеся в планировании мощностей
- **Ипотека Банк / Hamkorbank** — ML-приложения в банковском секторе
- **EPAM Узбекистан** — активно нанимает ML-инженеров для разработки fintech-решений
...
**Мухаммадали**
Степень бакалавра в области искусственного интеллекта и больших данных
Создаёт производственные ML-системы для растущей технологической экосистемы Узбекистана.
...
*Отказ от ответственности по данным: Данные об объёмах транзакций синтетически сгенерированы и откалиброваны по общедоступной агрегированной статистике ЦБ. Курсы USD/UZS получены напрямую из публичного API Центрального банка Узбекистана. Проект предназначен для портфолио и образовательных целей.*
