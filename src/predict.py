...
def make_prediction(target_date: str, model_name: str, models: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if len(window) < 30:
        raise ValueError(f"Insufficient history to predict for {target_date}. Need at least 30 days.")

    # Append the target date row
    ...
    target_row = {
        "date": target_dt,
        "transaction_volume": last_known_volume,     # placeholder
    ...
    # Take the last row for inference
    ...
    if target_type == "log_ratio_to_rolling_mean_30":
        ...
        if hasattr(raw_pred, "__len__"):
            raw_pred = raw_pred[0]
        predicted_volume = int(max(0, float(raw_pred)))

    # Detect Uzbekistan-specific events
